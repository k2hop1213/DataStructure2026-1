...
int main() {
	...
	for (int i = 0; s[i] != '\0'; i++) {
		...
		if ((cur >= 'A' && cur <= 'Z') || (cur >= '1' && cur <= '9')) { //피연산자일 때는 스택에 push한다
			...
			//연산자의 우선순위가 낮다면 괄호를 씌워줘야함 (우선순위가 낮지만 먼저 계산되었기 때문에)
			if (a1.p < cur_p) sprintf(s1, "(%s)", a1.str);
			else strcpy(s1, a1.str);
			//연산자 기준 뒤에 있는 피연산자와 비교할때는 등호를 넣는다
			//수식에서의 위치를 봤을 때 더 뒤에 있다면 우선순위가 같을 때라도 먼저 계산된것은 괄호를 씌워줘야함
			//ex) 후위표기식 ABC+- 를 중위로 변환하면 A-(B+C)인데 이때 조건문에 등호를 넣어주지 않으면 괄호가 씌워지지 않음 그래서 A-B+C가 되므로 틀린게 됨 
			//연산자 우선순위가 같지만 B+C가 먼저 계산되어야 하므로 괄호를 씌워주기 위해 조건문에 등호를 넣어서 처리한다
